...
class ScaleGraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        for i in range(self.k):
            self.gconv.append(
                GraphBlock(configs.c_out , configs.d_model , 32, 32,
                        2 , 0.3, 0.3 ,configs.seq_len,
                           10))
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # x_enc.shape = torch.Size([8, 12, 321])
        # x_mark_enc.shape = torch.Size([8, 12, 4])
        # x_dec.shape = torch.Size([8, 24, 321])
        # x_mark_dec.shape = torch.Size([8, 24, 4])
